    ...
    >
      ...
      <Box sx={{ overflowX: 'auto' }}>
        <Table size="small" sx={{ minWidth: 650 }}>
          <TableHead>
            <TableRow>
              <TableCell sx={{ fontWeight: 700, width: 70 }}>{t('monthly.year', { defaultValue: 'Рік' })}</TableCell>
              {MONTH_NAMES.map((m) => (
                <TableCell key={m} align="right" sx={{ px: 0.75, fontSize: '0.75rem', fontWeight: 600 }}>
                  {m}
                </TableCell>
              ))}
          ...
          <TableBody>
            {data.map((row) => {
              const yearPnlData = formatSignedPnl(row.totalPnl, currency, numberFormat, currentLang);
              return (
                <TableRow key={row.year} hover>
                  ...
                  {MONTH_NAMES.map((_, idx) => {
                    ...
                    const cellPnl = formatSignedPnl(item.pnl, currency, numberFormat, currentLang);
                        ...
                        <TableCell
                          ...
                          sx={{
                            ...
                            backgroundColor: (theme) =>
                              isPos ? theme.palette.trade.gainBg : isNeg ? theme.palette.trade.lossBg : 'transparent',
                            color: (theme) =>
                              isPos ? theme.palette.trade.gain : isNeg ? theme.palette.trade.loss : 'text.secondary',
                          }}
                        >
                          {cellPnl.text}
                        </TableCell>
