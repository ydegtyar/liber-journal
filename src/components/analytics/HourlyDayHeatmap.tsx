...
export const HourlyDayHeatmap: React.FC<HourlyDayHeatmapProps> = ({
  trades,
  currency,
  numberFormat,
}) => {
  ...
  const formatSlotLabel = (slot: SlotData) => {
      ...
      <Box sx={{ overflowX: 'auto', pb: 1 }}>
        <Box sx={{ minWidth: 920 }}>
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', mb: '3px' }}>
            <Box sx={{ width: 44, flexShrink: 0, textAlign: 'left', fontSize: '0.68rem', fontWeight: 700, color: 'text.secondary' }}>
              {t('heatmap.day')}
            </Box>
            <Box sx={{ display: 'flex', flex: 1, gap: '2px' }}>
              {Array.from({ length: 24 }, (_, h) => {
                ...
                if (isNY && isLondon) sessionColor = '#10b981'; // overlap
            ...
            {/* Total Col Header */}
            <Box sx={{ width: 68, flexShrink: 0, textAlign: 'right', fontSize: '0.68rem', fontWeight: 700, color: 'text.secondary', pr: 0.5 }}>
              {t('heatmap.total')}
            </Box>
          </Box>
          ...
          <Box sx={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
                ...
                <Box key={dIdx} sx={{ display: 'flex', alignItems: 'center' }}>
                  ...
                  <Box sx={{ display: 'flex', flex: 1, gap: '2px' }}>
                    {daySlots.map((slot) => {
                      ...
                      const tooltipContent = slot.tradesCount > 0 ? (
                        <Box sx={{ p: 0.5 }}>
                          <Typography variant="caption" sx={{ fontWeight: 700, display: 'block' }}>
                            {slot.dayName} · {String(slot.hour).padStart(2, '0')}:00 - {String(slot.hour).padStart(2, '0')}:59
                          </Typography>
                          <Typography
                            variant="caption"
                            sx={{
                              fontWeight: 800,
                              color: slot.pnl > 0 ? '#4ade80' : slot.pnl < 0 ? '#f87171' : 'inherit',
                              display: 'block',
                              fontSize: '0.82rem',
                            }}
                          >
                            {formatSignedPnl(slot.pnl, currency, numberFormat, currentLang).text}
                          </Typography>
                          <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                            {slot.tradesCount} trades ({slot.wins}W / {slot.losses}L / {slot.breakeven}BE)
                          </Typography>
                          <Typography variant="caption" sx={{ color: 'text.secondary', display: 'block' }}>
                        ...
                        <Tooltip key={slot.hour} title={tooltipContent} arrow placement="top">
                          <Box
                            sx={{
                              ...
                              borderRadius: '3px',
                              backgroundColor: bg,
                              border: (theme) => `1px solid ${slot.tradesCount > 0 ? 'rgba(0,0,0,0.1)' : theme.palette.divider}`,
                              display: 'flex',
                              alignItems: 'center',
                  ...
                  >
                    {metric === 'pnl' && formatCurrency(rowTotal.pnl, currency, numberFormat, currentLang)}
            ...
            <Box sx={{ display: 'flex', alignItems: 'center', mt: 0.5, pt: 0.5, borderTop: (theme) => `1px dashed ${theme.palette.divider}` }}>
              <Box sx={{ width: 44, flexShrink: 0, fontSize: '0.68rem', fontWeight: 800, color: 'text.secondary' }}>
                {t('heatmap.total')}
              </Box>
              ...
              <Box sx={{ display: 'flex', flex: 1, gap: '2px' }}>
                {colTotals.map((col) => {
                  ...
                  if (col.tradesCount > 0) {
                    if (metric === 'pnl') {
                      text = Math.abs(col.pnl) >= 1000 ? `${col.pnl >= 0 ? '+' : ''}${(col.pnl / 1000).toFixed(1)}k` : `${col.pnl >= 0 ? '+' : ''}${Math.round(col.pnl)}`;
              ...
              >
                {metric === 'pnl' && formatCurrency(overallTotal.pnl, currency, numberFormat, currentLang)}
