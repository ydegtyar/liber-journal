...
interface Props {
  ...
  dateBasis?: 'closed' | 'opened';
  onDateBasisChange?: (basis: 'closed' | 'opened') => void;
}

export const DailyPnlChart: React.FC<Props> = React.memo(
  ({ data, currency, numberFormat, dateBasis = 'closed', onDateBasisChange }) => {
          ...
          <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
            {onDateBasisChange && (
              <ButtonGroup size="small" variant="outlined">
                <Button
                  variant={dateBasis === 'closed' ? 'contained' : 'outlined'}
                  onClick={() => onDateBasisChange('closed')}
                  sx={{ fontSize: '0.75rem', py: 0.25, px: 1 }}
                >
                  {t('charts.byCloseDate', { defaultValue: 'Закриття' })}
                </Button>
                <Button
                  variant={dateBasis === 'opened' ? 'contained' : 'outlined'}
                  onClick={() => onDateBasisChange('opened')}
                  sx={{ fontSize: '0.75rem', py: 0.25, px: 1 }}
                >
            ...
            {data.length > 0 && (
              <Typography variant="caption" sx={{ color: '#eab308', fontWeight: 600 }}>
                {t('charts.averageDaily', { defaultValue: 'Середній' })}:{' '}
                {formatCurrency(avgDailyPnl, currency, numberFormat, currentLang)}
              </Typography>
            )}
          </Box>
        </Box>
